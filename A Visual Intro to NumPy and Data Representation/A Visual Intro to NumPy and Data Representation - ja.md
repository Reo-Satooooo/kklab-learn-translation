...
- モデルを処理そして構築するのに必要な全てのデータ型（スプレッドシート、画像、音声など…）について考えてみて下さい。それらの多くは、n次元配列での表現に完璧に適しています。

#### テーブルとスプレッドシート

- 値のスプレッドシートあるいは表は、2次元の行列です。スプレッドシート中の各シートは、それ独自の変数にする事が出来ます。それらのPythonで最も人気の抽象化はPandasデータフレームです。Pandasデータフレームでは、実際にNumPyを使用し、その上に構築します。
